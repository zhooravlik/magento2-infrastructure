...
from plugins.extensions.argparse import ArgumentParser
...
class Parsible(object):
    ...
    def __init__(self, input_file, parser, pid_file, debug, batch, batch_reload, auto_reload, stat_prefix):
        self.debug = debug
        self.batch = batch
        self.batch_reload = batch_reload
    ...
    def _run_periodic_functions(self):
        ...
        self.logger.debug('File Statistics: Current Byte Location {current}'.format(current=current))
        self.logger.debug('File Statistics: Current File Byte Size {size}'.format(size=size))
        self.logger.debug('File Statistics: Processed Percentage {percent:.2f} %'.format(percent=percent))
    ...
    def follow(self):
        ...
        if not (self.batch or self.batch_reload):
        ...
        while True:
            ...
            if not line:
                if self.batch:
                    ...
                    raise StopIteration
                if self.debug:
                    self.logger.debug('Tick Tock, waited for {0} iterations'.format(empty_iterations))
            ...
            try:
                yield self.parsing_function(line, self.stat_prefix)
            except Exception, e: 
                self.logger.debug(e)
...
if __name__ == '__main__':
    ...
    cmdline = ArgumentParser(usage="usage: parsible.py --log-file /var/log/mylog [options]",
    ...
    cmdline.add_argument('--batch-reload',
                         '-r',
                         action='store',
                         help='If Set, Parsible will start at the top of the log file, and continue to reload it once we reach the end.  Useful for processing logs where there is required headers at the top.',
                         dest='batch_reload',
                         default=False,
                         type=bool
                         )
    ...
    cmdline.add_argument('--stat-prefix',
                         '-sp',
                         action='store',
                         help="If Set, the prefix is passed to downstream parsers that may leverage the prefix as part of their stat naming convention.\n  \
                                This is useful where the parsed line may not have all the information needed to generate a valid stat name.",
                         dest='stat_prefix',
                         default=""
                        )
    ...
    if (args.batch_reload & args.batch):
        sys.stderr.write("\n[ERROR] Cannot choose both --batch-reload and --batch-mode Modes...\n\n")  
        sys.exit(1)

    p = Parsible(args.input_file, args.parser, args.pid_file, args.debug, args.batch, args.batch_reload, args.auto_reload, args.stat_prefix)
    p.main()
